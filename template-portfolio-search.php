	...
	global $wp_query; // will be reset to get pagination to work
        ...
        if($wp_query->have_posts()) {
        	...
        	while( $wp_query->have_posts() ): $wp_query->the_post();
	            ...
	            if ($thumb) {
		            ?>
		            <a data-posturl="<?php echo $currentPostLink; ?>" class="portfolio-item" data-productlink="<?php echo $product_link; ?>" data-refnumber="<?php echo $reference_number; ?>" data-lowres="<?php echo $thumb['sizes']['masonry-thumbnail']; ?>" data-highres="<?php echo $photo['url']; ?>" href="<?php echo $photo['url']; ?>" rel="gallery"><div style="display: none;" class="portfolio-desc"><?php echo $post_content; ?></div><img data-original="<?php echo $photo['url']; ?>" src="<?php echo $thumb['sizes'][$size]; ?>" srcset="<?php echo $thumb['sizes']['masonry-thumbnail']; ?> 1650w, <?php echo $thumb['sizes'][$size]; ?> 3000w" alt="<?php echo addslashes($photo['alt']); ?>" title="<?php echo addslashes($photo['caption']); ?>" class="lazy grid-item portfolio-image originalthumb <?php echo $size; ?>" /></a>
		            <?php
	            }
	/*            else {
	              if ($photo) {
